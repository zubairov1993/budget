import { ChangeDetectionStrategy, Component, Inject, OnInit, inject, OnDestroy, ChangeDetectorRef } from '@angular/core'
...
import { BudgetStateI, SharedService, updateMountlyBudgetAction, updateMountlyBudgetSuccessAction } from 'src/app/shared'
...
export class ChangeMountlyBudgetDialogComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    const monthlyBudget = this.sharedService.monthlyBudget$.subscribe(count => {
      this.currentMonthlyBudget = count
      this.form.controls['monthlyBudget'].setValue(this.currentMonthlyBudget)
      this.cdr.detectChanges()
    })
    const actions = this.actions$.pipe(ofType(updateMountlyBudgetSuccessAction), take(1)).subscribe(() => this.context.completeWith(this.form.value))
    this.allSubscription.push(monthlyBudget, actions)
  }

  submit(): void {
    if (this.form.controls['monthlyBudget'].value !== this.currentMonthlyBudget) {
      this.isLoading = true
      this.cdr.detectChanges()
