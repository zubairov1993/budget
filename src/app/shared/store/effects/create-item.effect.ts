...
export class CreateItemEffect {
  ...
  createItem$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ yearName, year, monthName, month, dayName, day, itemObj }) =>
        this.budgetService
          .createItem(yearName, monthName, dayName, itemObj)
          .pipe(
            map((response: { name: string }) => {
              return createItemSuccessAction({ year, month, day, itemObj });
            }),
            catchError((errorResponse: HttpErrorResponse) =>
              of(createItemFailureAction()),
            ),
          ),
      ),
    ),
  ...
  createitemAfterDay$ = createEffect(() =>
    this.actions$.pipe(
      ofType(createItemSuccessAction),
      map(({ year, month, day, itemObj }) => {
        return updateMountlyBudgetAction({
          year,
          monthlyBudget: itemObj.priceRu,
          bool: true,
        });
      }),
    ),
